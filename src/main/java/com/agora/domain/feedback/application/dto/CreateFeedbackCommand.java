...
public record CreateFeedbackCommand(
        ...
        @Schema(description = "Detailed feedback description", example = "Users are requesting a dark mode option for better usability in low-light environments. This would reduce eye strain and improve overall user experience.")
        @NotBlank(message = "Description cannot be blank")
        @Size(min = 10, max = 5000, message = "Description must be between 10 and 5000 characters")
        String description,

        @Schema(description = "Category ID for feedback classification", example = "117457749108987394")
        Long categoryId,

        @Schema(description = "Author/user ID who submitted the feedback", example = "117457749108987388")
        Long authorId,

        @Schema(description = "Sentiment of the feedback", example = "POSITIVE", enumeration = {"POSITIVE", "NEGATIVE", "NEUTRAL"})
        String sentiment,

        @Schema(description = "Comma-separated tags for categorization", example = "ui,accessibility,enhancement")
        String tags
) {
}
